    ...
    <div className="flex flex-col min-h-[calc(100vh-4rem)]">
      ...
      <header className="fixed top-16 left-0 w-full z-40 flex justify-between items-center px-margin-sm h-14 max-w-container-max-width mx-auto bg-surface/80 backdrop-blur-md">
        <button
          onClick={() => router.back()}
          aria-label="뒤로가기"
          className="w-12 h-12 flex items-center justify-start text-on-surface-variant hover:opacity-80 transition-opacity"
        >
      ...
      {/* 작성 영역 */}
      <main className="flex-1 flex flex-col w-full max-w-container-max-width mx-auto px-margin-sm pt-[7rem] pb-[6rem]">
        <div className="flex-1 flex flex-col bg-surface-container-lowest rounded-xl shadow-[0_4px_20px_0_rgba(147,74,46,0.04)] p-6 md:p-8 relative min-h-[300px]">
          <textarea
            ...
            autoFocus
            className="w-full h-full flex-1 bg-transparent border-none focus:ring-0 resize-none font-body-lg text-body-lg text-on-surface placeholder:text-outline/60 outline-none min-h-[250px]"
          />
      ...
      {/* 하단 버튼 */}
      <div className="fixed bottom-16 md:bottom-0 left-0 w-full z-40 bg-gradient-to-t from-surface via-surface to-transparent pt-8 pb-6 px-margin-sm max-w-container-max-width left-1/2 -translate-x-1/2">
        <button
          onClick={handleSubmit}
          disabled={!content.trim() || loading}
          className="w-full h-12 md:h-14 bg-primary text-on-primary rounded-full font-title-md text-title-md flex items-center justify-center hover:bg-surface-tint transition-all active:scale-95 shadow-md disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {loading ? '등록 중...' : '던지기'}
        </button>
      </div>
