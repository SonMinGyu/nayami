...
public class ConcernController {
  ...
  public ResponseEntity<ConcernResponse> create(
      ...
      @Valid @RequestBody ConcernCreateRequest request) {
    ConcernResponse concern = concernService.create(userId, request);
    concernService.publishCheckRequest(concern);
    return ResponseEntity.status(HttpStatus.CREATED).body(concern);
  }

  @GetMapping("/random")
  public ResponseEntity<ConcernResponse> findRandom(@AuthenticationPrincipal Long userId) {
    return concernService.findRandom(userId)
